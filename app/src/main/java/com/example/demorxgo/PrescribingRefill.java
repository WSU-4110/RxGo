...
public class PrescribingRefill extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        df.get ().addOnCompleteListener ( task -> {
            ...
            df2.get ().addOnCompleteListener ( task1 -> {
                ...
                df3.get().addOnCompleteListener ( task11 -> {
                    DocumentSnapshot document3 = task11.getResult ();
                    if (document3.exists ()) {
        ...
        send.setOnClickListener ( v -> {
            ...
            fStore.collection ( "patients" ).get().addOnCompleteListener ( task -> {
                ...
                for(int i = 0; i<patientsArrayList.size ();i++) {
                    ...
                    if (patientsArrayList.get ( i ).getLastName ().equals ( LName )&& patientsArrayList.get ( i ).getFirstName ().equals ( FName )&& patientsArrayList.get ( i ).getBirthday ().equals ( Bday )) {

                        int o = i; //variable for patient position in array

                        //getting Dr info to add to Rx info
                        fAuth = FirebaseAuth.getInstance ();
                        DocumentReference df1 = fStore3.collection ( "prescriber" ).document ( fAuth.getUid () );
                        df1.get ().addOnCompleteListener ( task12 -> {
                            ...
                            if (document.exists ()) {
                                ...
                                DocumentReference df2 = fStore2.collection ( "patients" ).document ( patientsArrayList.get ( o ).getId () ).collection ( "Prescriptions" ).document ( (int) (Math.random () * 10000000) + "" );//setting user Id from auth to match doc Id in store for new user
                                //building contents
                                ...
                                df2.set ( user ).addOnCompleteListener ( task121 -> {
                                    Log.d ( TAG, "added" );
